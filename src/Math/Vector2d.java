...
public class Vector2d {
    ...
    public Polar toPolar(Vector2d shift){
        int shiftedX = this.x - shift.x;
        int shiftedY = this.y - shift.y;
        double r = Math.sqrt((Math.pow((double)shiftedX,2))+(Math.pow((double)shiftedY,2)));
        double fi = Math.atan((double)shiftedX/shiftedY);
        return new Polar(r, fi);
    }
